...
export class PlaylistRepository {
  async save(playlist: Playlist): Promise<void> {
    ...
    await db.query<PlaylistRow>(
      `INSERT INTO ${TABLE_NAME} (id, name, owner, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5)
  ...
  async findAll(userId: string): Promise<Playlist[]> {
    const result = await db.query<PlaylistRow>(`SELECT * FROM ${TABLE_NAME} WHERE owner=$1`, [
      userId,
    ]);
    return result.rows.map((r) => mapPlaylistRowToEntity(r));
  }

  async findAllByIds(playlistIds: string[]): Promise<Playlist[]> {
    const result = await db.query<PlaylistRow>(
      `SELECT * FROM ${TABLE_NAME} WHERE id = ANY($1::text[])`,
      [playlistIds],
    );
    return result.rows.map((r) => mapPlaylistRowToEntity(r));
  }
  ...
  async delete(playlist: Playlist): Promise<boolean> {
    const primitive = playlist.toPrimitives();
    await db.query(`DELETE FROM ${TABLE_NAME} WHERE id=$1 RETURNING *`, [primitive.id]);
